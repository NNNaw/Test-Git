...
module.exports.Register = async function (req, res) {
  try {
    ...
    let accountType = await pool
      .request()
      .query(
        `SELECT * from LoaiTaiKhoan where idLoai = ${req.body.idLoaiTaiKhoan}`
      );
...
module.exports.Login = async function (req, res) {
  try {
    ...
    let accountExist = await pool.request()
      .query(`SELECT * from dbo.TaiKhoan where taiKhoan 
    ...
    if (accountExist.recordset[0]) {
      ...
      let accountType = await pool
        .request()
        .query(
          `SELECT * from LoaiTaiKhoan where idLoai = ${accountExist.recordset[0].idLoai}`
        );
      let userDetail, userExist;

      switch (accountType.recordset[0].tenLoai) {
        case "Sinh Viên":
          userExist = await pool
            .request()
            .query(
              `SELECT * from SinhVien where taiKhoan = '${accountExist.recordset[0].taiKhoan}'`
            );
          userDetail = {
        ...
        case "Giảng Viên":
          userExist = await pool
            .request()
            .query(
              `SELECT * from GiangVien where taiKhoan = '${accountExist.recordset[0].taiKhoan}'`
            );
          userDetail = {
            idLoaiTaiKhoan: accountType.recordset[0].idLoai,
            tenLoaiTaiKhoan: accountType.recordset[0].tenLoai,
